...
function isPinterestEmbedUrl(url: string): boolean {
    return /^https:\/\/assets\.pinterest\.com\/ext\/embed\.html\?id=\d+$/i.test(url)
}
...
export function FrameComponent({
    ...
    const mediaIsVideo = isVideoUrl(video)
    const mediaIsPinterestEmbed = isPinterestEmbedUrl(video)
    ...
    }, [isHovered, autoplayMode, mediaIsVideo])
                ...
                >
                    ...
                    >
                        {mediaIsVideo ? (
                            ...
                            <img src={video} alt={label} className="w-full h-full object-cover" />
                        )}
                    </div>
                </div>
